...
export default async function getAppVault (kmsId, edvId) {
    ...
    const {mnemonic : appMenemonic } = await global.kmsVault.getDecryptedDocument(kmsId)
    Logger.log('Inside getAppVault(), menemonic = ' + appMenemonic ,'getAppVault');
    const appVaultWallet = await VaultWalletManager.getWallet(appMenemonic);
    const appVault = await EdvClientManagerFactoryService.createEdvClientManger(appVaultWallet, edvId)
    return appVault;
}
