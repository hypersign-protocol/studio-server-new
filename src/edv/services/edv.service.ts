...
export class EdvService {
    ...
    constructor(private readonly configService: ConfigService){
     ...
     async setAuthenticationKey(Ed25519VerificationKey2020KeyPair: IEd25519VerificationKey2020KeyPair, authenticationKeyId: string, controller: string) {
     ...
     async hypersignDIDKeyResolverForEd25519KeyPair({ id }) {
        ...
        let authserverDid: any = fs.readFileSync(process.env.EDV_DID_FILE_PATH).toString()
        authserverDid = JSON.parse(authserverDid)
        let authserverKey: any = fs.readFileSync(process.env.EDV_KEY_FILE_PATH).toString()
